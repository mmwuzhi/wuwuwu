...
<CH.Section>
[_`ref.current`_](focus://17[7:17],24[18:28]) 就算放到 [_`useEffect`_](focus://14[3:11]) 的deps里或者作为组件的props，也不会在变化的时候触发组件重新渲染或者 [_`useEffect`_](focus://14[3:11]) 的第一个参数的再执行
...
<CH.Section>
想在父组件使用子组件中定义的函数或者取得子组件中定义的值的时候，可以在子组件里定义[第二个参数(_`ref`_)](focus://6[78:80],12[23:25])和[_`useImperativeHandle`_](focus://12[3:21])将内部的函数或者值传递给外部

然后使用[_`forwardRef`_](focus://20[16:26,41])包住目标子组件
...
```
这里将组件的type定义为[_`ForwardRefRenderFunction<ChildComponentHandlers>`_](focus://6[23:70])之后再在[_`export default`_](focus://20[1:14])的时候包住子组件，是因为如果在定义子组件的地方直接用 _`forwardRef`_ 包住的话，eslint会提示**component definition is missing display name forwardref**
</CH.Section>

<CH.Section>
修改完子组件之后，就可以在父组件将[_`useRef`_](focus://#2[29:34])定义的[_`ref`_](focus://#2[9:25])传递给子组件，然后就可以用用这个[_`ref`_](focus://#2[9:25])来调用子组件内定义的函数或者获取子组件内部定义的值了
